...
export default function Home() {
  ...
  useEffect(() => {
    // Check for saved search term from header
  ...
  useEffect(() => {
    // Filter jobs when search term changes (only for tradesperson)
    if (user?.role === 'tradesperson' && searchTerm.trim()) {
      ...
      <main className="pt-24 pb-16 px-4 md:px-8 max-w-7xl mx-auto">
        ...
        {user?.role === 'tradesperson' && searchTerm && (
          <div className="mb-4 text-sm text-gray-600">
            Found {displayJobs.length} result{displayJobs.length !== 1 ? 's' : ''} for "{searchTerm}"
            <button
              onClick={() => {
                setSearchTerm('');
                setFilteredJobs(jobs);
              }}
              className="ml-2 text-trade-primary hover:underline"
            >
              Clear search
            </button>
          </div>
